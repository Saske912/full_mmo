...
- **Staging, smoke (факт):** 2026-03-28 — образ **`abb3c8b`** (git-корень монорепо `MMO/`); `deploy-staging.sh` (**`goose-migrate-job`** → **`X-MMO-Goose-Version: 20260428120000`**) + `staging-verify.sh` с **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`** OK (**`gateway-api-smoke`**, ws-smoke, B2). Контент: **`scholar_deep_archive`** / **`scholar_sigil`**; **`target_progress`** / ветки (**`branch_path_*`**, **`mercenary_elite_contract`**); JSONB-**`inventory`**; **`/readyz`** при БД. Gateway: **`prerequisite_quest_id`**, **`newly_unlocked_quests`**, **`items/remove`**, **`items/transfer`**, **`resolve-preview`**; при расхождении last_cell с resolve — **HTTP 409** до WebSocket upgrade + заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](deploy/terraform/staging/gateway_migrations.auto.tfvars) + **`STAGING_VERIFY_READYZ_GOOSE_AFTER_DEPLOY`** / **`make verify-readyz-goose`**.
- **Клиент Unity:** каталог **`../Unity/`** относительно backend в монорепо: `make unity-proto` → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**, **`MmoClientWorldState`**, **`EventSystem`** при старте); сцена **`Assets/MMO/Scenes/MmoPlayground.unity`** (первая в Build Settings); меню **`GameObject/MMO/Playground Bootstrap`**; EditMode-тесты **`Assets/MMO/Tests`** (DTO + handoff preview); Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**. **Playground e2e (проверено):** WS, **WASD** (новый Input System), **`POST /v1/me/items/remove`** с реальным предметом (**`tutorial_shard`**) OK; квест — id из сессии (**`tutorial_intro`** и др.).
