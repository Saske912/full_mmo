...
- **Staging, smoke (факт):** 2026-03-28 — образ **`c8c2427`** (git `main`, монорепо `MMO/`); `deploy-staging.sh` (**`goose-migrate-job`** → **`X-MMO-Goose-Version: 20260428120000`**) + `staging-verify.sh` с **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`** OK (**`gateway-api-smoke`**, ws-smoke, B2). Контент: **`scholar_deep_archive`** / **`scholar_sigil`**; **`target_progress`** / ветки (**`branch_path_*`**, **`mercenary_elite_contract`**); JSONB-**`inventory`**; **`/readyz`** при БД. Gateway: **`prerequisite_quest_id`**, **`newly_unlocked_quests`**, **`items/remove`**, **`items/transfer`**, **`resolve-preview`**; при расхождении last_cell с resolve — **HTTP 409** до WebSocket upgrade + заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](deploy/terraform/staging/gateway_migrations.auto.tfvars) + **`STAGING_VERIFY_READYZ_GOOSE_AFTER_DEPLOY`** / **`make verify-readyz-goose`**.
- **Клиент Unity:** каталог **`../Unity/`** относительно backend в монорепо: `make unity-proto` → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**, **`MmoClientWorldState`**, **`EventSystem`** при старте); **WebGL:** **`Plugins/WebGL/MmoWebSocket.jslib`**, приём кадров через **`MmoWorldStreamClient.Poll()`** в **`Update`**; сцена **`Assets/MMO/Scenes/MmoPlayground.unity`** (первая в Build Settings); меню **`GameObject/MMO/Playground Bootstrap`**; EditMode-тесты **`Assets/MMO/Tests`**; Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**. **Playground e2e:** WS (**Standalone/Editor** — `ClientWebSocket`), **WASD**, клики по **строкам инвентаря** → поле **item_id**, **`items/remove`**, квесты (**`tutorial_intro`**).
- **Клиент Playground — шаги проверки (ручные, staging):**
  1. Открыть сцену **`MmoPlayground`**, Play в Editor или Standalone build.
  2. **Адрес gateway:** `https://mmo.pass-k8s.ru` (или свой); **player_id** уникальный; **resolve_x / resolve_z** при необходимости (например `0,0` или `-500,-500` для child-sw).
  3. **Подключиться к миру** — статус WS открыт; мета показывает золото, инвентарь, квесты.
  4. **WASD** — куб сущности двигается; при необходимости в инспекторе **`MmoGameBootstrap`**: **Position Extrapolation** (`0…1`) для лёгкого предсказания.
  5. Под списком меты — **кнопки строк инвентаря**: клик подставляет **item_id** в поле «Предмет (удалить)» → **Удалить 1 шт.** (ожидать `items/remove: ok` или ошибку нехватки).
  6. **Квест:** поле id из меты → **Отправить +1 к прогрессу квеста**.
  7. **Handoff / смена соты:** при **409** на WS — сценарий из **`MmoGameBootstrap`** (повторная сессия с **last_cell**); полный auto-reconnect без переподключения — см. runbook.
  8. **WebGL:** **File → Build Settings → WebGL** → билд; публикация только с **https**; к тому же gateway URL — **wss** (mixed content при `http`+`wss` не взлетит). Консоль браузера на ошибки CORS/WebSocket.
...
- **Криптоэкономика (дизайн):** токен **BET**, NFT (soulbound / продаваемые), **burn**, газлесс‑релей, мультисиг — на уровне продуктового дизайна; **смарт‑контракты, релей и on-chain синк в коде пока не реализованы**. Канон: [`../docs/crypto-economy.md`](../docs/crypto-economy.md).
...
1. **Unity:** полировка интерполяции (prediction/extrapolation по желанию); UX инвентаря (клики по строкам вместо ручного id); **WebGL** — отдельный транспорт (сейчас WS только Standalone/Editor).
2. **Postgres / баланс:** развитие контента (**`branch_*`**, награды, экономика), нагрузочные сценарии; CI и пост-деплой: [`docs/ci-and-deploy.md`](docs/ci-and-deploy.md), [`../.github/workflows/backend-ci.yml`](../.github/workflows/backend-ci.yml).
3. **Соты:** live-handoff и кандидаты миграции (**`ListMigrationCandidates`** / `mmoctl migration-dry-run`); операторский §**7–8** [runbook](runbooks/cold-cell-split.md), **`scripts/run-forward-npc-handoff.sh`**.
4. **Observability:** **`MMO_CELL_OTEL_TICK_SPAN`** только под расследование; [**`grafana-dashboard-grid-rpc-p95-by-method.json`**](deploy/observability/grafana-dashboard-grid-rpc-p95-by-method.json) — реимпорт в Grafana при новом инстансе (uid **`mmo-grid-rpc-p95`**).
