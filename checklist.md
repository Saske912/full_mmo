...
- **Клиент Unity:** каталог **`Unity/`** (рядом с backend в монорепо): `make unity-proto` (в каталоге `backend/`) → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**, **`MmoClientWorldState`**, сессия/WS/handoff, интерполяция кубов, **`EventSystem` + `InputSystemUIInputModule` в `Awake`**); сцена **`Assets/MMO/Scenes/MmoPlayground.unity`** (первая в Build Settings); EditMode-тесты **`Assets/MMO/Tests`** (DTO + handoff preview); меню **`GameObject/MMO/Playground Bootstrap`**; Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**. **Playground e2e (проверено):** WS, **WASD**, **`items/remove`** (**`tutorial_shard`**), квесты — id из сессии (**`tutorial_intro`**).
