...
## Снимок состояния (март 2026, обновлено 2026-03-29 — актуализация репо/приоритетов)
...
- **Staging, smoke (факт):** 2026-03-29 — **репозиторий `mmo` `main`** **`826b92b`**: миграция баланса [`20260429120000_balance_tutorial_scholar_rewards.sql`](../backend/internal/db/migrations/20260429120000_balance_tutorial_scholar_rewards.sql); **[`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md)** — preflight и регрессия п.3 (dry-run in-cluster, handoff). **Суперпроект `full_mmo` `c979eac`**. **Unity `7e1ab31`** — playground: подсветка строк инвентаря, синхронизация с полем удаления, обновление меты после quest/remove, дефолты экстраполяции (**~0.18** / cap **2.5 м**). **Образ в кластере** — после очередного **`deploy-staging.sh`** с этим **`mmo`**; до выката **`/readyz`** мог показывать **`X-MMO-Goose-Version: 20260428120000`**, после Job **`/migrate`** с новым образом — **`20260429120000`**. Выкат: при пустом state — импорт существующих ресурсов **`mmo`** → **`scripts/deploy-staging.sh`** (тесты → build/push → **`goose-migrate-job.sh`** → **`tofu apply`** на **`gateway`**, **`grid-manager`**, **`cell-node`**, шарды из **`cell_instances`**). Предусловия: **`deploy/terraform/staging/backend.tf`** — в репозитории [`backend.tf`](../backend/deploy/terraform/staging/backend.tf) (как у [`backend.tf.example`](../backend/deploy/terraform/staging/backend.tf.example)); **`certs/fullchain.pem`** + **`certs/privkey.pem`**, **`kubectl`**, remote state для **`data.terraform_remote_state.staging`**. Кластер **Talos**, **`mmo`**, каталог **`cell_0_0_0`** + **`cell_-1_-1_1`**. **`staging-verify.sh`** (в т.ч. **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`**, **`EXPECT_CELL_IDS`**, **`RESOLVE_CHECKS`**) к **`https://mmo.pass-k8s.ru`** — OK. **`Makefile`:** **`SHELL := /bin/bash`**; **`harbor-login`** / **`goose-migrate-job.sh`** — см. [ci-and-deploy.md](../backend/docs/ci-and-deploy.md). **Контент (пример):** **`scholar_*`**, **`branch_path_*`**, **`mercenary_elite_contract`**; награды tutorial/scholar см. миграцию **`20260429120000`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](../backend/deploy/terraform/staging/gateway_migrations.auto.tfvars); **`verify-readyz-goose`** / **`STAGING_VERIFY_READYZ_GOOSE_AFTER_DEPLOY`**.
- **Клиент Unity:** каталог **`Unity/`** (submodule рядом с **`backend/`**, актуальный git — см. пункт **Staging** выше): `make unity-proto` (в каталоге `backend/`) → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`** partial: Editor/Standalone — **`HttpClient`**, **WebGL player** — **`UnityWebRequest`** в **`MmoGatewayClient.WebGL.cs`**; **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**, **`MmoClientWorldState`**, сессия/WS/handoff, интерполяция, **`EventSystem` + `InputSystemUIInputModule` в `Awake`**); **WebGL:** [`Unity/Assets/Plugins/WebGL/MmoWebSocket.jslib`](../Unity/Assets/Plugins/WebGL/MmoWebSocket.jslib), **`Poll()`** в **`Update`**; сцена **`Assets/MMO/Scenes/MmoPlayground.unity`**; меню **`GameObject/MMO/Playground Bootstrap`**; тесты **`Assets/MMO/Tests`**. **Playground e2e:** WS, **WASD**, клик по строке инвентаря (подсветка выбранного **item_id**) → **`items/remove`**, квесты (**`tutorial_intro`**): **`progress`** в API — абсолютный, клиент шлёт следующий шаг из меты **`GET /v1/session`**.
...
- **Kubernetes:** кластер **Talos**; приложение в namespace **`mmo`** ([`deploy/terraform/staging/main.tf`](../backend/deploy/terraform/staging/main.tf)). State стека приложения в backend **kubernetes** (секрет в **`state`**; в репозитории — [`backend.tf`](../backend/deploy/terraform/staging/backend.tf) / [`backend.tf.example`](../backend/deploy/terraform/staging/backend.tf.example)): при **пустом** state и уже живом кластере сначала **`tofu import`** (иначе **`apply`** получит конфликты имён). Переменная **`cell_instances`**, **`mmo_structured_logs`** (по умолчанию **true** — env **`MMO_LOG_FORMAT=json`** на gateway / grid-manager / cell), опционально **`mmo_loki_log_labels`** для селекторов сбора логов. Пример двух сот: [`cell_instances.auto.tfvars`](../backend/deploy/terraform/staging/cell_instances.auto.tfvars).
