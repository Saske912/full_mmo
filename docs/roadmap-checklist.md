...
- **Staging, smoke (факт):** 2026-03-29 — **репозиторий `mmo` `main`** **`826b92b`**, образ в кластере **`harbor.pass-k8s.ru/library/mmo-backend:826b92b`**, **`GET /readyz`** → **`X-MMO-Goose-Version: 20260429120000`** (миграция баланса [`20260429120000_balance_tutorial_scholar_rewards.sql`](../backend/internal/db/migrations/20260429120000_balance_tutorial_scholar_rewards.sql)); **`staging-verify.sh`** (+ опц. **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`**) — OK. **[`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md)** — preflight / п.3. **Unity `7e1ab31`** — playground (инвентарь, экстраполяция, см. снимок). Выкат: **`scripts/deploy-staging.sh`** (тесты → build/push → **`goose-migrate-job.sh`** → **`tofu apply`**). Предусловия: [`backend.tf`](../backend/deploy/terraform/staging/backend.tf), серты Ingress, **`kubectl`**, remote state **`data.terraform_remote_state.staging`**. Кластер **Talos**, **`mmo`**, **`cell_0_0_0`** + **`cell_-1_-1_1`**. **`Makefile`:** **`SHELL := /bin/bash`**; [ci-and-deploy.md](../backend/docs/ci-and-deploy.md). **Контент:** награды tutorial/scholar после goose — см. **`20260429120000`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](../backend/deploy/terraform/staging/gateway_migrations.auto.tfvars).
...
1. **Unity / WebGL:** ручной **e2e** на **WebGL**-билде по расширенному [webgl-staging-e2e.md](webgl-staging-e2e.md) (**шаг 8** снимка); при желании — prediction (**`MmoGameBootstrap`**). WS: [`MmoWebSocket.jslib`](../Unity/Assets/Plugins/WebGL/MmoWebSocket.jslib), **`MmoWorldStreamClient`**, **`Poll()`**; HTTP — **`MmoGatewayClient.WebGL.cs`**.
