...
## Снимок состояния (март 2026, обновлено 2026-03-31)
...
- **Автоматика «скейла мира» (staging, факт):** grid-manager под флагами Terraform **`grid_manager_extra_env`** — оркестрация **split** (**`MMO_GRID_AUTO_SPLIT_WORKFLOW`**, **`MMO_GRID_AUTO_SPLIT_DRAIN`**, load policy, NATS **`grid.split.workflow`**, multi-child **`ForwardNpcHandoff`**) и **merge / scale-in** (**`MMO_GRID_AUTO_MERGE_WORKFLOW`**, **`MMO_GRID_MERGE_*`**, **`ForwardMergeHandoff`**, **`grid.merge.workflow`**). **Live handoff игроков при merge:** **`MMO_GRID_MERGE_PLAYER_HANDOFF`** (+ **`MAX_PLAYERS`**). Идентификаторы дочерних сот — **path-based** (`cell_root`, `cell_q0`, `cell_q0_q3`, …), см. [`partition/split`](../backend/internal/partition/split.go). **Gateway:** переключение downstream при транспортных ошибках к соте; на staging по умолчанию **`GATEWAY_ALLOW_CELL_HANDOFF_MISMATCH`** (Terraform **`gateway_allow_cell_handoff_mismatch`**) — не блокировать вход при stale **`mmo_player_last_cell`** после merge. **Ручной cold-path** ([`cold-cell-split.md`](../backend/runbooks/cold-cell-split.md) §6–7, §5 вывод родителя) — **архивный fallback** (инциденты, регрессии «строгого» 409, окружения без авто-workflow), не основной путь эксплуатации на staging.
- **Staging, smoke (факт):** 2026-03-29 — **репозиторий `mmo` `main`**, образ в Harbor **`library/mmo-backend`** (тег в [`image.auto.tfvars`](../backend/deploy/terraform/staging/)); **`GET /readyz`** → **`X-MMO-Goose-Version: 20260429120000`** (миграция баланса [`20260429120000_balance_tutorial_scholar_rewards.sql`](../backend/internal/db/migrations/20260429120000_balance_tutorial_scholar_rewards.sql)); **`staging-verify.sh`** (+ опц. **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`**, **`STAGING_VERIFY_RESET_AUTO_CELLS=1`** при «засорённом» каталоге после split-e2e) — OK. **[`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md)** — preflight, merge/split, логи. **Unity `7e1ab31`** — playground (инвентарь, экстраполяция, см. снимок). Выкат: **`scripts/deploy-staging.sh`**. Предусловия: [`backend.tf`](../backend/deploy/terraform/staging/backend.tf), серты Ingress, **`kubectl`**, remote state. Кластер **Talos**, **`mmo`**: в Terraform **`cell_instances`** — **baseline только primary** (**`cell_root`**); дочерние соты для split — **runtime** (**cell-controller** → `cell-node-auto-*` / `mmo-cell-auto-*`, subject **`cell.control`**). **`Makefile`:** **`SHELL := /bin/bash`**; [ci-and-deploy.md](../backend/docs/ci-and-deploy.md). **Контент:** награды tutorial/scholar после goose — см. **`20260429120000`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](../backend/deploy/terraform/staging/gateway_migrations.auto.tfvars).
...
- **Клиент Playground — шаги проверки (ручные, staging):**
  ...
  7. **409 / handoff:** реконнект и сценарий **last_cell** в **`MmoGameBootstrap`**; на staging gateway чаще пускает без `409` при mismatch (**`GATEWAY_ALLOW_CELL_HANDOFF_MISMATCH`**). Подробнее — [runbook §4](../backend/runbooks/cold-cell-split.md), [cells-migration-workflow](../backend/docs/cells-migration-workflow.md).
...
- **Grafana (staging):** дашборд **MMO Backend (staging)** — UID **`mmo-backend-overview`**: панели rate/**p95** (gateway, grid registry, cell tick). Дополнительно импорт JSON **p95 по `method`** для **`mmo_grid_registry_rpc_duration_seconds`**: [`deploy/observability/grafana-dashboard-grid-rpc-p95-by-method.json`](../backend/deploy/observability/grafana-dashboard-grid-rpc-p95-by-method.json) (uid **`mmo-grid-rpc-p95`**). **Замечание:** на этом же гистограмме **`method="ForwardMergeHandoff"`** при авто-merge даёт **высокий p95/p99 (секунды)** при **умеренном p50** — ожидаемый хвост, см. [`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md) (подраздел про латентность **ForwardMergeHandoff**). Группа алертов **`mmo-staging`**: ошибки RPC/apply_input и **SLO по p95** — **`mmo_gateway_registry_resolve_duration_seconds`** > 0.5s; **`mmo_gateway_cell_join_duration_seconds`** > 0.5s; **`mmo_cell_tick_step_duration_seconds`** > 50ms; **`mmo_grid_registry_rpc_duration_seconds`** (`method="ForwardCellUpdate"`) > 0.5s; **`ForwardNpcHandoff`** (правило **`mmostgp95npch`**, `namespace="mmo"` в PromQL) > 0.5s (окна 5m; серия оживёт после реальных вызовов RPC). Образец PrometheusRule: [`deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml`](../backend/deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml); примеры LogQL: [`deploy/observability/loki-logql-traceid.example.txt`](../backend/deploy/observability/loki-logql-traceid.example.txt). **Логи:** Terraform **`mmo_structured_logs`** + **`mmo_loki_log_labels`**, outputs **`loki_logql_base`** / **`loki_pod_label_selectors`**, [`internal/logging`](../backend/internal/logging).
- **Grid-manager (Registry):** **`ForwardCellUpdate`** → `Cell.Update`; режимы **noop**, **tps**, **split-prepare**, **split-drain true|false**, **`export-npc-persist`**, **`import-npc-persist`**; **`ForwardNpcHandoff`** / **`ForwardMergeHandoff`** (handoff NPC при split и merge; **`mmoctl forward-npc-handoff`**, **`forward-merge-handoff`**). Условия import — как у **`import_npc_persist`** ([runbook](../backend/runbooks/cold-cell-split.md) §6). **`mmoctl migration-dry-run`**. **`PlanSplit`** / **`partition-plan`**. Метрики: **`mmo_grid_registry_rpc_*`**, **`mmo_grid_manager_split_workflow_*`**, **`mmo_grid_manager_merge_workflow_*`**; при **`metrics-listen`** — агрегаты по сотам **`mmo_grid_manager_cell_*`** (см. пункт мониторинга нагрузки).
- **Распил B2 (каталог):** `ResolveMostSpecific` выбирает соту с **максимальным `level`** среди содержащих точку; тест [`internal/discovery/split_resolve_test.go`](../backend/internal/discovery/split_resolve_test.go); в **`staging-verify.sh`** при появлении в registry дочерней (например **`cell_q0`**) проверяется `resolve -500 -500` → эта сота; при одной соте шаг пропускается с подсказкой (child можно получить **`make split-e2e-smoke`**).
- **Эпик B3 (масштабирование сот):** **основной путь** — авто **split/merge** в grid-manager + **cell-controller** (см. «автоматика скейла мира» выше); смоуки **`split-e2e`**, **`merge-e2e`**, **`grid-full-cycle-e2e`**, **`merge-live-players-e2e`**. Операторский [**runbook**](../backend/runbooks/cold-cell-split.md) и ручной **`mmoctl forward-npc-handoff`** / **`forward-merge-handoff`** — **архив / fallback** (инциденты, окружения без авто-workflow). **Gateway:** **`GET /v1/me/resolve-preview`**; **409** при mismatch last_cell ↔ resolve — если **`GATEWAY_ALLOW_CELL_HANDOFF_MISMATCH`** выключен; на staging по умолчанию включён ([`staging/README.md`](../backend/deploy/terraform/staging/README.md)). Полный seamless WS без участия клиента — вне scope; частичное смягчение — transport retry / перерезолв в gateway.
...
2. **B3 — хвосты после автоматики скейла:** операторский **§5** ([runbook](../backend/runbooks/cold-cell-split.md)) — вывод baseline parent из каталога / Terraform при необходимости «чистого» контура; teardown runtime child — **`MMO_GRID_SPLIT_TEARDOWN_RUNTIME_CHILDREN`**. Улучшение клиентского seamless-handoff — по желанию. Ручные §6–7 runbook **не приоритезировать** в спринте, пока нет регресса авто-workflow — держать как **архив**.
...
**Эпик B3 — автоматика скейла мира (staging, март 2026):**

На staging **основной путь** — **`MMO_GRID_AUTO_SPLIT_WORKFLOW`** + **`MMO_GRID_AUTO_MERGE_WORKFLOW`** (и связанные **`MMO_GRID_*`** в [`grid_manager.auto.tfvars`](../backend/deploy/terraform/staging/grid_manager.auto.tfvars)); baseline в Terraform — **`cell_root`**; runtime-дети — **cell-controller** (**`cell.control`** после **`PlanSplit`**). Документация: [**`cells-migration-workflow.md`**](../backend/docs/cells-migration-workflow.md), Terraform [`staging/README.md`](../backend/deploy/terraform/staging/README.md). Ручной **cold-path** ([runbook §6–7](../backend/runbooks/cold-cell-split.md)) — **архив** для fallback; полный вывод baseline parent (§5) — по необходимости, не ежедневная процедура.

| Шаг | Статус | Действие |
|-----|--------|----------|
| Игроки | Авто + частичный UX | Seamless без реконнекта **нет**; **`resolve-preview`**; на staging по умолчанию **`GATEWAY_ALLOW_CELL_HANDOFF_MISMATCH`** — вход не режется **409** при stale **`last_cell`** после merge; при выключенном флаге — **409** и runbook §4. Плюс retry/перерезолв в gateway при transient gRPC. |
| Вывод родителя | Операция по runbook §5 при необходимости | **Архив / редкий шаг:** graceful shutdown → deregister; **`cell_instances`** только для baseline (**`cell_root`**); runtime child не коммитить в tfvars. |
| NPC handoff | Авто в grid-manager | В workflow: **`ForwardNpcHandoff`** (split), **`ForwardMergeHandoff`** (merge). Ручной **`mmoctl forward-*`** — **архив** для отладки. |
| Чистка после смоуков | Рекомендация | **`STAGING_VERIFY_RESET_AUTO_CELLS=1`** в **`staging-verify.sh`**; **`split-e2e-smoke`** — **`RESET_AUTO_CHILDREN_BEFORE_TEST`** (default **1**). |
...
- [x] **Cold-path сплита (B3) — [архив / fallback]:** [runbook](../backend/runbooks/cold-cell-split.md), `partition-plan`; **основной путь эксплуатации** — **`MMO_GRID_AUTO_SPLIT_WORKFLOW`**, **`MMO_GRID_AUTO_POST_HANDOFF_ORCHESTRATION`**, cell-controller, см. снимок «автоматика скейла мира».
- [x] Исполняемый **авто** split/merge в grid-manager: **`split_drain`**, **`split_prepare`**, **`PlanSplit`**, multi-child **`ForwardNpcHandoff`**, **`ForwardMergeHandoff`**, merge handoff при **`MMO_GRID_MERGE_PLAYER_HANDOFF`**, path-based **`cell_q*`**; **полная live-миграция игроков без реконнекта** — [ ] *(остаётся вне scope; gateway: transient retry / перерезолв, опц. **`GATEWAY_ALLOW_CELL_HANDOFF_MISMATCH`** на staging)*.
...
- [x] gRPC **прокси на соты:** `Registry.ForwardCellUpdate` → `Cell.Update` (**`mmoctl forward-update`**: noop, tps, split-*, export/import-npc-persist); **`Registry.ForwardNpcHandoff`**, **`Registry.ForwardMergeHandoff`** (**`mmoctl forward-npc-handoff`**, **`forward-merge-handoff`**); **`migration-dry-run`**
- [x] Политики нагрузки и **оркестрация split + merge** из grid-manager: breach/cooldown, **`mmo_grid_manager_load_policy_actions_total`**, **`MMO_GRID_AUTO_SPLIT_DRAIN`**, **`MMO_GRID_AUTO_SPLIT_WORKFLOW`**, **`MMO_GRID_AUTO_MERGE_WORKFLOW`**, NATS **`grid.split.workflow`** / **`grid.merge.workflow`**, метрики **`mmo_grid_manager_split_workflow_*`**, **`mmo_grid_manager_merge_workflow_*`**. Конфигурация — **`grid_manager_extra_env`** ([`staging/README.md`](../backend/deploy/terraform/staging/README.md)); ручной цикл runbook §6–7 — **архив**.
- [x] Логирование операций registry (`internal/grpc/registrysvc`: `Register`, `ListCells`, `ResolvePosition`, `ForwardCellUpdate`, `doForwardCellUpdate`, `ForwardNpcHandoff`, `ForwardMergeHandoff`; structured `slog` start/done/error)
...
- [ ] **ForwardMergeHandoff — интерпретация p95/p99:** на **`mmo-grid-rpc-p95`** зафиксирован «толстый» хвост (секунды) при волнах merge при низком QPS и субсекундном типичном runtime — см. [`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md). Следующий шаг при необходимости: бюджет/отдельное правило алерта или разбиение латентности по стадиям (трассы/логи), без смешения с SLO по **ForwardCellUpdate**.
...
**Снимок 2026-03:** MVP **авто split/merge** (grid-manager + cell-controller, Consul-каталог, path-based id, handoff NPC, merge с игроками по флагам) уже на staging — см. буллет **«автоматика скейла мира»** в разделе **«Снимок состояния»** выше и [`cells-migration-workflow.md`](../backend/docs/cells-migration-workflow.md). Разделы **2.1–2.3** ниже — **целевое** состояние (Redis-топология, NATS `cell.migration.*` как единый транспорт сущностей, двухфазный коммит, «незаметный» redirect клиента); часть пунктов **перекрыта** текущей реализацией или отложена.
