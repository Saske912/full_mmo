...
- **Staging, smoke (факт):** 2026-03-29 — образ **`ec7a764`** (`harbor.pass-k8s.ru/library/mmo-backend`, git **`mmo` `main`**); выкат: **docker build** → push в Harbor → **`kubectl set image`** для **`gateway`**, **`grid-manager`**, **`cell-node`**, **`cell-node-child-sw`** (локальный **`tofu apply`** без `certs/*.pem` в рабочем дереве не использовался — полный пайплайн см. **`deploy-staging.sh`** на машине с remote state и TLS). Кластер **Talos** / **`mmo`**, каталог **`cell_0_0_0`** + **`cell_-1_-1_1`**. **`staging-verify.sh`** (in-cluster migration-dry-run, **`STAGING_VERIFY_EXPECT_CELL_IDS`**, **`STAGING_VERIFY_RESOLVE_CHECKS`**, **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`**) — OK; **`run-forward-npc-handoff.sh`** in-cluster — OK; **`make load-smoke`** (40 сессий, gateway burst) — OK. **`X-MMO-Goose-Version: 20260428120000`**. Контент и gateway API — как в предыдущем снимке (**`resolve-preview`**, 409/handoff-заголовки и т.д.). **Makefile:** **`harbor-login`** / **`harbor-push`** — fallback **`HARBOR_REGISTRY_HOSTNAME`** / **`HARBOR_DOCKER_*`** и фильтр мусора от **`tofu output`**.
...
1. **Unity:** полировка движения (prediction по желанию); UX инвентаря (клики по строкам; после remove/quest — обновление меты из сессии). **WebGL:** отдельный транспорт для **мирового WS** — [`MmoWebSocket.jslib`](../Unity/Assets/Plugins/WebGL/MmoWebSocket.jslib), **`Poll()`** в bootstrap; чеклист билда и **https/wss** — [webgl-staging-e2e.md](webgl-staging-e2e.md); при проблемах с **HTTP** — сверить **`MmoGatewayClient`** (WebGL vs **HttpClient** в Editor).
